...
class Ui(QtWidgets.QMainWindow):
    ...
    def saveSettings(self):
        ...
        try:
            if self.outroPlaceholder_4.text() and self.outroPlaceholder_3.text() and self.outroPlaceholder_2.text() and self.outroPlaceholder.text() and self.mainPlaceholder.text():
                ...
                cur.execute(q, (
                self.mainPlaceholder.text(), self.outroPlaceholder.text(), self.outroPlaceholder_2.text(),
                self.outroPlaceholder_3.text(), self.outroPlaceholder_4.text(), self.bodyPlaceholder.text(),
                self.popupPlaceholder.text(), self.popupPlaceholder_2.text(), self.popupPlaceholder_3.text(),
                self.popupPlaceholder_4.text(), self.musicPlaceholder.text(), self.fontsPlaceholder.text(),
                self.outputPlaceholder.text(), checkbox1, checkbox2, checkbox3))
                cur.execute('commit')
                ...
                cur.execute(q, (
                self.mainPlaceholder.text(), self.outroPlaceholder.text(), self.outroPlaceholder_2.text(),
                self.outroPlaceholder_3.text(), self.bodyPlaceholder.text(), self.popupPlaceholder.text(),
                self.popupPlaceholder_2.text(), self.popupPlaceholder_3.text(), self.musicPlaceholder.text(),
                self.fontsPlaceholder.text(), self.outputPlaceholder.text(), checkbox1, checkbox2, checkbox3))
                cur.execute('commit')
                self.renderStatusPlaceholder.setText("Settings Saved")
                ...
                cur.execute('DROP TABLE SETTINGS')
                cur.execute(
                    "CREATE TABLE IF NOT EXISTS SETTINGS (MAINVID VARCHAR(1000), OUTROVID VARCHAR(1000),OUTROVID2 VARCHAR(1000),MAINTEXT VARCHAR(1000),POPTEXT VARCHAR(1000),POPTEXT2 VARCHAR(1000),MUSICDIR VARCHAR(1000),FONTDIR VARCHAR(1000),OUTPUTDIR VARCHAR(1000), BOX1 VARCHAR , BOX2 VARCHAR ,BOX3 VARCHAR )")
                q = "INSERT INTO SETTINGS VALUES(?,?,?,?,?,?,?,?,?,?,?,?)"
                cur.execute(q, (
                self.mainPlaceholder.text(), self.outroPlaceholder.text(), self.outroPlaceholder_2.text(),
                self.bodyPlaceholder.text(), self.popupPlaceholder.text(), self.popupPlaceholder_2.text(),
                self.musicPlaceholder.text(), self.fontsPlaceholder.text(), self.outputPlaceholder.text(), checkbox1,
                checkbox2, checkbox3))
                cur.execute('commit')
                self.renderStatusPlaceholder.setText("Settings Saved")
                ...
                cur.execute('DROP TABLE SETTINGS')
                cur.execute(
                    "CREATE TABLE IF NOT EXISTS SETTINGS (MAINVID VARCHAR(1000), OUTROVID VARCHAR(1000),MAINTEXT VARCHAR(1000),POPTEXT VARCHAR(1000),MUSICDIR VARCHAR(1000),FONTDIR VARCHAR(1000),OUTPUTDIR VARCHAR(1000), BOX1 VARCHAR , BOX2 VARCHAR ,BOX3 VARCHAR )")
                q = "INSERT INTO SETTINGS VALUES(?,?,?,?,?,?,?,?,?,?)"
                cur.execute(q, (self.mainPlaceholder.text(), self.outroPlaceholder.text(), self.bodyPlaceholder.text(),
                                self.popupPlaceholder.text(), self.musicPlaceholder.text(),
                                self.fontsPlaceholder.text(), self.outputPlaceholder.text(), checkbox1, checkbox2,
                                checkbox3))
                cur.execute('commit')
                self.renderStatusPlaceholder.setText("Settings Saved")
        except:
    ...
    def operation(self):
        ...
        remove_audio = self.checkBox_10.isChecked()
        ...
        len_pop = len(popPathFiles)
        music_check = self.checkBox_9.isChecked()
        if int(music_check):
            len_music = len(musicPathFiles)
        else:
            len_music = 1
        ...
        if int(check3) and int(check2) and int(check1):
            t = (
                        len_main * len_outro * len_outro2 * len_outro3 * len_outro4 * len_body * len_pop * len_pop2 * len_pop3 * len_pop4)
        ...
        if int(remove_audio):
            for ele in os.listdir("mainTemp"):
                source = "mainTemp/" + ele
                dest = "mainTemp/" + get_random_string(10) + ".mp4"
                q = "ffmpeg -f lavfi -i anullsrc=channel_layout=stereo:sample_rate=44100 -i {} -c:v copy -c:a aac -shortest {}".format(
                    source, dest)
                print(q)
                try:
                    os.system(f"""{q}""")
        ...
        if int(remove_audio2):
            for ele in os.listdir("outroTemp"):
                source = "outroTemp/" + ele
                dest = "outroTemp/" + get_random_string(10) + ".mp4"
                q = "ffmpeg -f lavfi -i anullsrc=channel_layout=stereo:sample_rate=44100 -i {} -c:v copy -c:a aac -shortest {}".format(
                    source, dest)

                # q = "ffmpeg -f lavfi -i anullsrc -i {} -c:v copy -c:a aac -map 0:a -map 1:v -shortest {}".format(source,dest)
                try:
                    os.system(f"""{q}""")
                except:
                    pass
        ...
        if int(remove_audio3):
            for ele in os.listdir("outroTemp2"):
                source = "outroTemp2/" + ele
                dest = "outroTemp2/" + get_random_string(10) + ".mp4"
                # q = "ffmpeg -f lavfi -i anullsrc -i {} -c:v copy -c:a aac -map 0:a -map 1:v -shortest {}".format(source,dest)
                q = "ffmpeg -f lavfi -i anullsrc=channel_layout=stereo:sample_rate=44100 -i {} -c:v copy -c:a aac -shortest {}".format(
                    source, dest)
                print(q)
        ...
        if int(remove_audio4):
            for ele in os.listdir("outroTemp3"):
                source = "outroTemp3/" + ele
                dest = "outroTemp3/" + get_random_string(10) + ".mp4"
                # q = "ffmpeg -f lavfi -i anullsrc -i {} -c:v copy -c:a aac -map 0:a -map 1:v -shortest {}".format(source,dest)
                q = "ffmpeg -f lavfi -i anullsrc=channel_layout=stereo:sample_rate=44100 -i {} -c:v copy -c:a aac -shortest {}".format(
                    source, dest)
        ...
        if int(remove_audio5):
            for ele in os.listdir("outroTemp4"):
                source = "outroTemp4/" + ele
                dest = "outroTemp4/" + get_random_string(10) + ".mp4"
                # q = "ffmpeg -f lavfi -i anullsrc -i {} -c:v copy -c:a aac -map 0:a -map 1:v -shortest {}".format(source,dest)
                q = "ffmpeg -f lavfi -i anullsrc=channel_layout=stereo:sample_rate=44100 -i {} -c:v copy -c:a aac -shortest {}".format(
                    source, dest)
                        ...
                        for b in t5:
                            if not int(music_activate):
                                if current_videos_in_output == total_requiredvideos:
                                    break
                            file = open('li.txt', 'w')
                            x1, y1, z1, a1, b1, fname = f"mainTemp\{x}", f"outroTemp\{y}", f"outroTemp2\{z}", f"outroTemp3\{a}", f"outroTemp4\{b}", get_random_string(
                                10)
                            file.write(
        ...
        if int(music_activate):
            print("MUSIC HAS BEEN ACTIVATED AND IS DOING THE THINGS")
            current_videos_in_output = 0
            ...
            for x in tempmusicpathfiles:
                ...
                for y in m2:
                    x1 = f"tempmusic\{x}"
                    y1 = f"merged\{y}"
                    fname = outputPath + "/" + get_random_string(10) + ".mp4"
                    ...
                    try:
                        ...
                        q = "ffmpeg -i \"{}\" -stream_loop -1 -i \"{}\" -c:v copy -filter_complex \"[0:a]aformat=fltp:44100:stereo,apad[0a];[1]aformat=fltp:44100:stereo,volume={}[1a];[0a][1a]amerge[a]\" -map 0:v -map \"[a]\" -ac 2 -shortest -y {}".format(
                            y1, x1, dial, fname)
                        os.system(f"""{q}""")
                        print(q)
                    except:
